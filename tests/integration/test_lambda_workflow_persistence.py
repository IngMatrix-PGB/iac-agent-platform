...
LambdaResourceSpec -> real LangGraph workflow (real
`AWSResourceRenderer` dispatching to the real Lambda renderer, real
Terraform, the real trusted `terraform/modules/lambda` module, real
...
Mirrors tests/integration/test_dynamodb_workflow_persistence.py
...
def test_real_workflow_state_survives_sqlite_checkpointer_and_graph_reconstruction(
    ...
    request_id = "req-lambda-durable-001"
    ...
    assert recovered["resource_spec"].name == "orders-processor"

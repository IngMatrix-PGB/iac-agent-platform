...
DynamoDB pipeline (Batch 17).

DynamoDBResourceSpec -> real LangGraph workflow (real
`AWSResourceRenderer` dispatching to the real DynamoDB renderer, real
Terraform, the real trusted `terraform/modules/dynamodb` module, real
plan analyzer, real platform policy, real Checkov, real security gate)
-> SQLite checkpoint -> CLOSE -> new checkpointer + new compiled graph
against the SAME database file -> get_state(same thread config) ->
recovered interrupted state.

Mirrors tests/integration/test_s3_workflow_persistence.py exactly, via
`build_iac_workflow`. The original saver and graph objects are never
reused for recovery — this is what makes the test a genuine simulation
of process reconstruction, not just "the graph remembered it in
memory".
"""
...
def test_real_workflow_state_survives_sqlite_checkpointer_and_graph_reconstruction(
    ...
    request_id = "req-ddb-durable-001"
    ...
    assert recovered["resource_spec"].name == "orders-events-table"

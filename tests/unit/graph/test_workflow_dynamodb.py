...
DynamoDB exactly as it does for SQS/S3 (see tests/unit/graph/test_workflow.py
and tests/unit/graph/test_workflow_s3.py).
...
This file instead proves the DynamoDB-specific seams: `render_terraform`
picks the DynamoDB renderer/trusted module dir, DynamoDB's own platform
policies route PASS/WARN/BLOCK correctly, `checkov_scan` selects
DynamoDB's own (empty) Checkov profile, and `source_control` produces
DynamoDB-labeled (mixed-case "DynamoDB", not "DYNAMODB") PR/commit
text — via `build_iac_workflow` directly, not a resource-specific
...
# ---------------------------------------------------------------------------
# Fakes (deliberately duplicated from test_workflow.py/test_workflow_s3.py —
# same precedent as the SQS/S3/DynamoDB contract and renderer test files
# each owning their own fixtures).
# ---------------------------------------------------------------------------

_DEFAULT_DYNAMODB_PLAN_JSON = {
    "terraform_version": "1.16.1",
    "resource_changes": [
        {
            "address": "module.dynamodb.aws_dynamodb_table.this",
...
class FakeTerraformRunner:
    def __init__(self, *, plan_json=None):
        self.calls: list[str] = []
        self._plan_json = plan_json if plan_json is not None else _DEFAULT_DYNAMODB_PLAN_JSON
...
def _build(
    ...
    *,
    ...
    graph = build_iac_workflow(
        renderer=AWSResourceRenderer(dynamodb_renderer=renderer),
...
def test_happy_path_node_order_and_final_status(tmp_path):
    ...
    result = graph.invoke({"request_id": "req-ddb-001", "resource_spec": _spec()})
...
def test_renderer_called_once_with_correct_spec(tmp_path):
    ...
    spec = _spec()
    graph.invoke({"request_id": "req-ddb-001", "resource_spec": spec})

    assert len(renderer.render_calls) == 1
    assert renderer.render_calls[0]["spec"] == spec


def test_render_uses_the_dynamodb_trusted_module_dir(tmp_path):
    graph, renderer, _, _, _ = _build(tmp_path)
    graph.invoke({"request_id": "req-ddb-001", "resource_spec": _spec()})

    module_source = renderer.render_calls[0]["module_source"]
    assert "dynamodb" in module_source
    assert "sqs" not in module_source
    assert module_source.strip("./").split("/")[-1] != "s3"
...
def test_platform_evaluation_contains_all_three_dynamodb_policies(tmp_path):
    graph, _, _, _, _ = _build(tmp_path)
    result = graph.invoke({"request_id": "req-ddb-001", "resource_spec": _spec()})
...
def test_pitr_disabled_gives_warn_but_still_routes_to_approval(tmp_path):
    graph, _, _, _, _ = _build(tmp_path)
    result = graph.invoke(
        {"request_id": "req-ddb-001", "resource_spec": _spec(point_in_time_recovery=False)}
    )
...
def test_deletion_protection_disabled_gives_warn_but_still_routes_to_approval(tmp_path):
    graph, _, _, _, _ = _build(tmp_path)
    result = graph.invoke(
        {"request_id": "req-ddb-001", "resource_spec": _spec(deletion_protection=False)}
    )
...
def test_checkov_block_routes_to_blocked_with_no_interrupt(tmp_path):
    ...
    block_result = CheckovScanResult(
        findings=(
            SecurityFinding(
                policy_id="CKV_AWS_119",
                severity=SecuritySeverity.HIGH,
                status=PolicyStatus.BLOCK,
                resource="module.dynamodb.aws_dynamodb_table.this",
                message="Checkov CKV_AWS_119 failed.",
...
def test_dynamodb_request_gets_its_own_approved_checkov_profile(tmp_path):
    """Batch 17 regression: DynamoDB receives its own approved profile
    (exactly CKV_AWS_119, the customer-managed-KMS non-goal) — never
    S3's four skips, never a hardcoded adapter default."""
    graph, _, _, checkov, _ = _build(tmp_path)
    graph.invoke({"request_id": "req-ddb-001", "resource_spec": _spec()})

    expected = checkov_profile_for(ResourceType.DYNAMODB)
    assert len(checkov.profiles_seen) == 1
    assert checkov.profiles_seen[0] == expected
    assert checkov.profiles_seen[0].skipped_checks == ("CKV_AWS_119",)


# ---------------------------------------------------------------------------
# Resume through source_control (no DynamoDB-specific branch there at all)
# ---------------------------------------------------------------------------


def _invoke_to_interrupt(workspace_root, saver, request_id, spec, **overrides):
    graph = build_iac_workflow(
        renderer=AWSResourceRenderer(dynamodb_renderer=overrides.get("renderer") or FakeRenderer()),
...
def test_approve_resume_gives_pr_created(tmp_path):
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-ddb-approve", _spec(), source_control_port=source_control
        )
...
def test_reject_resume_gives_rejected_status(tmp_path):
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(workspace_root, saver, "req-ddb-reject", _spec())
...
def test_pr_body_states_resource_type_dynamodb(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    source_control = FakeSourceControl()
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-ddb-body", _spec(), source_control_port=source_control
        )
        graph.invoke(Command(resume=ApprovalDecision.APPROVE.value), config)

    pr_body = source_control.calls[0]["pr_body"]
    assert "Resource type: dynamodb" in pr_body
    assert "Resource type: s3" not in pr_body
    assert "Resource type: sqs" not in pr_body


def test_commit_message_says_dynamodb_mixed_case_not_all_caps(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    source_control = FakeSourceControl()
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-ddb-commit", _spec(), source_control_port=source_control
        )
        graph.invoke(Command(resume=ApprovalDecision.APPROVE.value), config)

    commit_message = source_control.calls[0]["commit_message"]
    assert commit_message == "feat(iac): add DynamoDB proposal req-ddb-commit"
...
def test_interrupted_dynamodb_state_round_trips_through_real_sqlite(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    config = workflow_config("req-ddb-durable-001")

    with open_sqlite_checkpointer(db_path) as saver:
        graph = build_iac_workflow(
            renderer=AWSResourceRenderer(dynamodb_renderer=FakeRenderer()),
        ...
        first_result = graph.invoke(
            {"request_id": "req-ddb-durable-001", "resource_spec": _spec()}, config
        )
    ...
    assert recovered["request_id"] == "req-ddb-durable-001"
    assert isinstance(recovered["resource_spec"], DynamoDBResourceSpec)

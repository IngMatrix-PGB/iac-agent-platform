"""Unit tests proving the generalized LangGraph workflow works for a
`ServerlessWorkerSpec` composition request exactly as it does for a
single AWS resource (see test_workflow.py, test_workflow_s3.py,
test_workflow_dynamodb.py, test_workflow_lambda.py).
...
composition-specific seams: `render_terraform` dispatches to the
serverless-worker renderer via `IacRenderer`, the composition's own
platform policies route PASS/WARN/BLOCK correctly, `checkov_scan`
selects the composition's own approved Checkov profile, and
`source_control` produces composition-labeled PR/commit text — via
`build_iac_workflow` directly, no new graph node.
...
# ---------------------------------------------------------------------------
# Fakes (deliberately duplicated from the SQS/S3/DynamoDB/Lambda workflow
# test files — same precedent as every resource-specific workflow test
# file owning its own fixtures).
# ---------------------------------------------------------------------------

_DEFAULT_COMPOSITION_PLAN_JSON = {
...
class FakeTerraformRunner:
    def __init__(self, *, plan_json=None):
        self.calls: list[str] = []
        self._plan_json = plan_json if plan_json is not None else _DEFAULT_COMPOSITION_PLAN_JSON
...
def _build(
    ...
    *,
    ...
    graph = build_iac_workflow(
        ...
        serverless_worker_renderer=renderer,
...
def test_happy_path_node_order_and_final_status(tmp_path):
    ...
    result = graph.invoke({"request_id": "req-worker-001", "resource_spec": _spec()})
...
def test_renderer_called_once_with_correct_spec(tmp_path):
    ...
    spec = _spec()
    graph.invoke({"request_id": "req-worker-001", "resource_spec": spec})
...
def test_render_uses_all_three_constituent_trusted_module_dirs(tmp_path):
    graph, renderer, _, _, _ = _build(tmp_path)
    graph.invoke({"request_id": "req-worker-001", "resource_spec": _spec()})

    module_sources = renderer.render_calls[0]["module_sources"]
    assert "sqs" in module_sources.queue
...
def test_platform_evaluation_contains_all_eight_composition_policies(tmp_path):
    graph, _, _, _, _ = _build(tmp_path)
    spec = _spec(
...
def test_composition_request_gets_its_own_approved_checkov_profile(tmp_path):
    """Proves the composition receives its own six-check profile — the
    union of DynamoDB's and Lambda's already-approved skips, verified
    empirically to be the correct union (see
    `iac_agent.security.composition_checkov_profiles`) — never S3's."""
    graph, _, _, checkov, _ = _build(tmp_path)
    graph.invoke({"request_id": "req-worker-001", "resource_spec": _spec()})

    expected = composition_checkov_profile_for(CompositionType.SQS_LAMBDA_DYNAMODB)
    assert len(checkov.profiles_seen) == 1
    assert checkov.profiles_seen[0] == expected
    assert "CKV_AWS_18" not in checkov.profiles_seen[0].skipped_checks
    ...
    block_result = CheckovScanResult(
        findings=(
            SecurityFinding(
                ...
                resource="module.function.aws_lambda_function.this",
...
# ---------------------------------------------------------------------------
# Resume through source_control (composition-labeled PR/commit text)
# ---------------------------------------------------------------------------
...
def _invoke_to_interrupt(workspace_root, saver, request_id, spec, **overrides):
    graph = build_iac_workflow(
        renderer=AWSResourceRenderer(),
        serverless_worker_renderer=overrides.get("renderer") or FakeRenderer(),
...
def test_approve_resume_gives_pr_created(tmp_path):
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-worker-approve", _spec(), source_control_port=source_control
        )
...
def test_reject_resume_gives_rejected_status(tmp_path):
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(workspace_root, saver, "req-worker-reject", _spec())
...
def test_pr_body_states_composition_type_and_the_three_names(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    source_control = FakeSourceControl()
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-worker-body", _spec(), source_control_port=source_control
        )
        graph.invoke(Command(resume=ApprovalDecision.APPROVE.value), config)

    pr_body = source_control.calls[0]["pr_body"]
    assert "Composition type: sqs_lambda_dynamodb" in pr_body
    assert "Queue: orders-queue" in pr_body
    assert "Lambda: orders-processor" in pr_body
    assert "DynamoDB: orders-table" in pr_body
    assert "Resource type:" not in pr_body
    # Never a raw Terraform dump.
    assert 'resource "aws_' not in pr_body


def test_commit_message_says_serverless_worker(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    source_control = FakeSourceControl()
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-worker-commit", _spec(), source_control_port=source_control
        )
        graph.invoke(Command(resume=ApprovalDecision.APPROVE.value), config)

    commit_message = source_control.calls[0]["commit_message"]
    assert commit_message == "feat(iac): add serverless worker proposal req-worker-commit"
...
def test_interrupted_composition_state_round_trips_through_real_sqlite(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    config = workflow_config("req-worker-durable-001")

    with open_sqlite_checkpointer(db_path) as saver:
        graph = build_iac_workflow(
            renderer=AWSResourceRenderer(),
            serverless_worker_renderer=FakeRenderer(),
        ...
        first_result = graph.invoke(
            {"request_id": "req-worker-durable-001", "resource_spec": _spec()}, config
        )
    ...
    assert recovered["request_id"] == "req-worker-durable-001"

...
Lambda exactly as it does for SQS/S3/DynamoDB (see
tests/unit/graph/test_workflow.py, test_workflow_s3.py,
test_workflow_dynamodb.py).
...
This file instead proves the Lambda-specific seams: `render_terraform`
picks the Lambda renderer/trusted module dir, Lambda's own platform
policies route PASS/WARN/BLOCK correctly, `checkov_scan` selects
Lambda's own approved Checkov profile, and `source_control` produces
Lambda-labeled PR/commit text — via `build_iac_workflow` directly, no
new graph node for the Lambda+IAM multi-resource relationship (the
trusted module owns that internally).
...
tests/integration/test_lambda_workflow_integration.py.
"""
...
# ---------------------------------------------------------------------------
# Fakes (deliberately duplicated from the SQS/S3/DynamoDB workflow test
# files — same precedent as the resource-specific contract and renderer
# test files each owning their own fixtures).
# ---------------------------------------------------------------------------

_DEFAULT_LAMBDA_PLAN_JSON = {
    "terraform_version": "1.16.1",
    "resource_changes": [
        {
            "address": "module.lambda.aws_cloudwatch_log_group.this",
            "change": {"actions": ["create"], "before": None, "after": {}},
        },
        {
            "address": "module.lambda.aws_iam_role.this",
            "change": {"actions": ["create"], "before": None, "after": {}},
        },
        {
            "address": "module.lambda.aws_iam_role_policy.logs",
            "change": {"actions": ["create"], "before": None, "after": {}},
        },
        {
            "address": "module.lambda.aws_lambda_function.this",
            "change": {"actions": ["create"], "before": None, "after": {}},
        },
    ],
}
...
_CLEAN_CHECKOV_RESULT = CheckovScanResult(
    findings=(), passed_checks=36, failed_checks=0, skipped_checks=0, scanner_version="3.3.13"
)
...
class FakeTerraformRunner:
    def __init__(self, *, plan_json=None):
        self.calls: list[str] = []
        self._plan_json = plan_json if plan_json is not None else _DEFAULT_LAMBDA_PLAN_JSON
...
def _build(
    ...
    *,
    ...
    graph = build_iac_workflow(
        renderer=AWSResourceRenderer(lambda_renderer=renderer),
...
def test_happy_path_node_order_and_final_status(tmp_path):
    ...
    result = graph.invoke({"request_id": "req-lambda-001", "resource_spec": _spec()})
...
def test_renderer_called_once_with_correct_spec(tmp_path):
    ...
    spec = _spec()
    graph.invoke({"request_id": "req-lambda-001", "resource_spec": spec})

    assert len(renderer.render_calls) == 1
    assert renderer.render_calls[0]["spec"] == spec


def test_render_uses_the_lambda_trusted_module_dir(tmp_path):
    graph, renderer, _, _, _ = _build(tmp_path)
    graph.invoke({"request_id": "req-lambda-001", "resource_spec": _spec()})

    module_source = renderer.render_calls[0]["module_source"]
    assert "lambda" in module_source
    assert "sqs" not in module_source
    assert "dynamodb" not in module_source
...
def test_platform_evaluation_contains_all_three_lambda_policies(tmp_path):
    graph, _, _, _, _ = _build(tmp_path)
    result = graph.invoke(
        {"request_id": "req-lambda-001", "resource_spec": _spec(reserved_concurrency=5)}
    )
...
def test_tracing_pass_through_gives_warn_but_still_routes_to_approval(tmp_path):
    graph, _, _, _, _ = _build(tmp_path)
    result = graph.invoke(
        {
            "request_id": "req-lambda-001",
            "resource_spec": _spec(tracing_mode=LambdaTracingMode.PASS_THROUGH),
        }
    )
...
def test_checkov_block_routes_to_blocked_with_no_interrupt(tmp_path):
    ...
    block_result = CheckovScanResult(
        findings=(
            SecurityFinding(
                policy_id="CKV_AWS_45",
                severity=SecuritySeverity.HIGH,
                status=PolicyStatus.BLOCK,
                resource="module.lambda.aws_lambda_function.this",
                message="Checkov CKV_AWS_45 failed.",
...
def test_lambda_request_gets_its_own_approved_checkov_profile(tmp_path):
    """Batch 18 regression: Lambda receives its own approved five-check
    profile — never S3's/DynamoDB's skips, never a hardcoded adapter
    default."""
    graph, _, _, checkov, _ = _build(tmp_path)
    graph.invoke({"request_id": "req-lambda-001", "resource_spec": _spec()})

    expected = checkov_profile_for(ResourceType.LAMBDA)
...
# ---------------------------------------------------------------------------
# Resume through source_control (no Lambda-specific branch there at all)
# ---------------------------------------------------------------------------


def _invoke_to_interrupt(workspace_root, saver, request_id, spec, **overrides):
    graph = build_iac_workflow(
        renderer=AWSResourceRenderer(lambda_renderer=overrides.get("renderer") or FakeRenderer()),
...
def test_approve_resume_gives_pr_created(tmp_path):
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-lambda-approve", _spec(), source_control_port=source_control
        )
...
def test_reject_resume_gives_rejected_status(tmp_path):
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(workspace_root, saver, "req-lambda-reject", _spec())
...
def test_pr_body_states_resource_type_lambda(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    source_control = FakeSourceControl()
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-lambda-body", _spec(), source_control_port=source_control
        )
        graph.invoke(Command(resume=ApprovalDecision.APPROVE.value), config)

    pr_body = source_control.calls[0]["pr_body"]
    assert "Resource type: lambda" in pr_body
    assert "Resource type: dynamodb" not in pr_body


def test_commit_message_says_lambda_mixed_case_not_all_caps(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    source_control = FakeSourceControl()
    ...
    with open_sqlite_checkpointer(db_path) as saver:
        graph, config = _invoke_to_interrupt(
            workspace_root, saver, "req-lambda-commit", _spec(), source_control_port=source_control
        )
        graph.invoke(Command(resume=ApprovalDecision.APPROVE.value), config)

    commit_message = source_control.calls[0]["commit_message"]
    assert commit_message == "feat(iac): add Lambda proposal req-lambda-commit"
...
def test_interrupted_lambda_state_round_trips_through_real_sqlite(tmp_path):
    db_path = tmp_path / "state.db"
    workspace_root = tmp_path / "workspaces"
    workspace_root.mkdir()
    config = workflow_config("req-lambda-durable-001")

    with open_sqlite_checkpointer(db_path) as saver:
        graph = build_iac_workflow(
            renderer=AWSResourceRenderer(lambda_renderer=FakeRenderer()),
        ...
        first_result = graph.invoke(
            {"request_id": "req-lambda-durable-001", "resource_spec": _spec()}, config
        )
    ...
    assert recovered["request_id"] == "req-lambda-durable-001"
    assert isinstance(recovered["resource_spec"], LambdaResourceSpec)
